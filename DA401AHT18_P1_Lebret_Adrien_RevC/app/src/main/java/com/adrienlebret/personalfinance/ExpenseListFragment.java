...
public class ExpenseListFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mListView = v.findViewById(R.id.expenseLV);
        expenseDatabaseManager = new ExpenseDatabase(this.getActivity().getApplicationContext());
        ArrayList<Expense> expenseArrayList = expenseDatabaseManager.getAllExpense();
        ArrayList<String>list = new ArrayList<>();

        for (Expense expense:expenseArrayList){
            list.add(expense.getExpenseCatagory()+"         "+expense.getExpenseAmount()+"€         "+expense.getIexpenseDate()+"\n"+expense.getExpenseDescription());
        }
        ArrayAdapter<String> adapter=new ArrayAdapter<>(this.getActivity().getApplicationContext(),R.layout.expense_custom_listview, R.id.expenseTitleTVId,list);
        ...
        dateB = mDateTo.getText().toString(); // DateB is assigned to the value chosen by the user
    ...
    public void compareDate() throws ParseException{
        ...
        if (date1.compareTo(date2) <=0){
            expenseDatabaseManager = new ExpenseDatabase(getActivity().getApplicationContext());
            ArrayList<Expense> expenseArrayList = expenseDatabaseManager.getAllExpenseBetween(dateA,dateB);
            ArrayList<String>list = new ArrayList<>();
            for (Expense expense:expenseArrayList){
                list.add(expense.getExpenseCatagory()+"         "+expense.getExpenseAmount()+"€         "+expense.getIexpenseDate()+"\n"+expense.getExpenseDescription());
            }
            ArrayAdapter<String> adapter=new ArrayAdapter<>(getActivity().getApplicationContext(),R.layout.expense_custom_listview, R.id.expenseTitleTVId,list);
